//
//  GSColor.swift
//  chukapoka
//
//  Created by Demian Yoo on 6/2/25.
//

import SwiftUI

enum GSColor {
    static let primary: Color = Color("Primary1")
    static let secondary1: Color = Color("Secondary1")
    static let secondary2: Color = Color("Secondary2")
    static let secondary3: Color = Color("Secondary3")
    static let gray1: Color = Color("Gray1")
    static let gray2: Color = Color("Gray2") // caption
    static let gray3: Color = Color("Gray3") // placeholder
    static let black: Color = Color("Black1")
    static let white: Color = Color.white
    static let yellow: Color = Color.accentColor
    static let purple: Color = Color("Purple1")
    static let pink: Color = Color("Pink1")
    static let cardRed1: Color = Color("CardRed")
    static let cardRed2: Color = Color("CardRed2")
    static let cardShadow: Color = Color("CardShadow")
    static let cardYellow: Color = Color("CardYellow")
}
